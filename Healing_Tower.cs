...
public class Healing_Tower : MonoBehaviour, myInterface
{
    string name = "Healing Tower";
    ...
    private int[] upgrade_damage = {110, 120, 130, 140};
    private float[] upgrade_range = {1.5f, 1.5f, 1.6f, 1.6f, 1.7f};
    ...
    void attack(GameObject target) {
        ...
        if(target.GetComponent<enemyInterface>().getHealth() <= damage){
            kills++;
            if(player.GetComponent<Player>().getHealth() < player.GetComponent<Player>().getMaxHealth()){
                player.GetComponent<Player>().subtractHealth(-2);
            }
        }
